...
const titles: Record<
  Exclude<keyof SubgraphOwnerSubgraphsRow, 'key' | 'versionId' | keyof SubgraphStates>,
  string
> = {
  id: 'ID',
  image: 'Img',
    ...
    hasLinkedSubgraphs:
      !denied && currentVersion && active
        ? deploymentVersions.some(
            (x) =>
              x.subgraph.currentVersion && x.subgraph.id !== id && x.subgraphDeployment.id === deploymentId,
          )
        : false,
    versionId,

...
import { CuratorSubgraphsRow, columnsWidth, columns, transformToCsvRow } from './curator-subgraphs.model';
import { useCuratorSubgraphs } from './curator-subgraphs.service';
...
export const CuratorSubgraphs: React.FC<Props> = observer(({ id }) => {
  const [
    {
      currentPage,
      perPage,
    ...
    new TableViewModel<CuratorSubgraphsRow>(
      'curator-subgraphs',
      {
        orderBy: 'shares',
        orderDirection: 'asc',
  ...
  const rows = useMemo(
    () =>
      data
        ? compose(
            (xs: Array<CuratorSubgraphsRow>) => xs.slice(perPage * (currentPage - 1), perPage * currentPage),
            sortRows(sortParams),
          )(data)
        : [],

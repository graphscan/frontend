...
export const WithdrawButton: React.FC<Props> = ({
  ...
  const { mutate: withdraw, isLoading: isWithdrawing } = useWithdraw(indexerId, currentAddress);

  const { account } = useEthers();

  const isCurrentDelegator = delegatorId === currentAddress || delegatorId === account?.toLowerCase();
  ...
  const { currentEpoch, epochLength, currentEpochEndBlock, currentBlock } = data;

  const secondsRemainingUntilWithdraw =
    ((lockedUntil - currentEpoch) * epochLength - (currentBlock - currentEpochEndBlock + epochLength)) *
    13.29;
  ...
  return isWithdrawing ? (
    ...
    <DelegatorDelegationsButton
      ...
      disabled={!(lockedTokens > 0 && lockedUntil > 0 && isCurrentDelegator && currentEpoch >= lockedUntil)}
      onClick={handleClick}

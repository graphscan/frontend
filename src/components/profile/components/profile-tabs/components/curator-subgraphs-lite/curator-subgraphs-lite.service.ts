...
import { NameSignal, Signal, transformToRow } from './curator-subgraphs-lite.model';
...
export const useCuratorSubgraphs = (id: string) => {
  return useQuery(['curator-subgraphs', id], async () => {
    ...
    const nameSignalsRows = nameSignals.map((s) => transformToRow({ ...s, type: 'Auto-Migrate' }));
    const signalsRows = signals.map((s) => transformToRow({ ...s, type: 'Deployment Signal' }));

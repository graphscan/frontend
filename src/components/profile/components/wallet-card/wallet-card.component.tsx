import { TitleCell, Heading, TotalGrt, TotalValue, Stats } from './wallet-card.styled';
  ...
  return (
    <Container>
      <Stats>
        ...
        <TitleCell>
          <TotalGrt>
            <TotalValue data-tip={tooltipNumberContent(total)}>{formatNumber(total)}</TotalValue> GRT
          </TotalGrt>
        </TitleCell>
        <WalletCardCell title="In wallet" value={balance} isBalance />
        <WalletCardCell title="Curation" value={curation} delta={curationDelta} />
        <WalletCardCell title="Delegation" value={delegation} delta={delegationDelta} />

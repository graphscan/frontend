...
type Props<Row> = {
  ...
  sortingOptions: {
    ...
    unsortableKeys?: Array<keyof Row | "delegate" | "undelegate" | "withdraw">;
  ...
  const columns = useMemo(
    () =>
      _columns.map((column, id) => ({
        ...column,
        className:
          column.dataIndex === sortParams.orderBy
            ? "ant-table-column_sorted "
            : "",
        width: columnsWidth
          ? columnsWidth[resolution][id]
          : (column.width ?? 120),
        ...
        onCell: (row: Row, rowIndex?: number) => {
          ...
          return {
            className:
              rowIndex === hoveredRowIndex ? "ant-table-cell_row-hovered" : "",
          };
        },
      })),
    [

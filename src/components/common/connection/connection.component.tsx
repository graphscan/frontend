...
import { EndpointSwitcher } from './components/endpoint-switcher/endpoint-switcher.component';
...
export const Connection: React.FC<Props> = observer(({ showGlow = true }) => {
  ...
  const connect = useCallback(() => activateBrowserWallet(), [activateBrowserWallet]);
  ...
  useEffect(() => {
    const handleOutsideClick = (e: MouseEvent) => {
      if (showMenu && e.target instanceof Node && !ref.current?.contains(e.target)) {
        toggleMenu();
      }
    };
  ...
  const { endpointSwitchWhitelist } = getEnvVariables();

  const whitelist = endpointSwitchWhitelist?.split(',');
      ...
      {typeof id === 'string' && showMenu && (
        <>
          ...
          <Menu>
            <MenuHeader>
              <MenuUserpick>
                <Link href={{ pathname: '/profile', query: { id: currentAddress ?? id } }}>
                  <a>
              ...
              <section>
                <Heading>
                  <Link href={{ pathname: '/profile', query: { id: currentAddress ?? id } }}>
                    <a onMouseDown={preventDefault}>{clampMiddle(currentAddress ?? id)}</a>
                  </Link>
                </Heading>
            ...
            {whitelist?.includes(id.toLowerCase()) && (
              <MenuBody>
                <EndpointSwitcher />
              </MenuBody>
            )}

...
import { SubgraphCuratorsRow, columnsWidth, columns, transformToCsvRow } from './subgraph-curators.model';
...
export const SubgraphCurators: React.FC<Props> = observer(({ ids, deploymentId }) => {
  const [
    {
      currentPage,
      perPage,
  ...
  const rows = useMemo(
    () =>
      data
        ? compose(
            (xs: Array<SubgraphCuratorsRow>) => xs.slice(perPage * (currentPage - 1), perPage * currentPage),
            sortRows(sortParams),
          )(data)
        : [],

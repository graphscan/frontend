...
import { SubgraphDetailsLite, transform } from './subgraph-details-lite.model';
...
type SubgraphVersionResponse = {
  subgraphVersion: SubgraphDetailsLite;
};
...
export const useSubgraphDetails = ({ deploymentId, versionId, subgraphId }: SubgraphDetailsParams) => {
  return useQuery(['subgraph-details', deploymentId, versionId, subgraphId], async () => {
    ...
    const activeSubgraphCuratorsCount = nameSignals.filter((s) => Number(s.nameSignal) > 0).length;
    ...
    return {
      ...
      deploymentCuratorsCount: {
        active: activeSubgraphCuratorsCount + signals.filter((s) => Number(s.signal) > 0).length,

...
export type SubgraphAllocationLite = {
  id: string;
  indexer: {
    id: string;
...
export type SubgraphAllocationsRowLite = {
  id: string;
  indexer: string;
  name: string | null;
  key: string;
...
const titles: Record<
  Exclude<
    keyof SubgraphAllocationsRowLite,
    'key' | 'name' | 'indexingRewardCutAtClose' | 'potentialIndexingRewardCut' | 'lifetimeEpochs'
  >,
  string
> = {
  id: 'Allocation ID',
...
): Array<ColumnType<SubgraphAllocationsRowLite>> => [
  {
...
export const createTransformerToRow = (
  ...
  poi,
}: SubgraphAllocationLite): SubgraphAllocationsRowLite => {

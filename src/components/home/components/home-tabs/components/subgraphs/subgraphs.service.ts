...
import { Subgraph } from './subgraphs.model';
...
export const useSubgraphs = () => {
  ...
  return useQuery<Array<Subgraph>>(
    ['subgraphs'],
    async () => {
      if (!networkData) {
        return Promise.reject(new Error('Subgraphs request cannot be sent without network data.'));
      }

      const subgraphs = await fetchAllParallel(networkData.subgraphCount, fetchSubgraphs);

...
const titles: Record<
  Exclude<keyof SubgraphsRow, 'id' | 'versionId' | 'key' | 'favourite' | keyof SubgraphStates>,
  string
> = {
  image: 'Img',
  displayName: 'Name',
...
export const columns: Array<ColumnType<SubgraphsRow>> = [
  ...
  {
    title: createTitleWithTooltipDescription(titles.stakedTokens, 'Sum of GRT allocated by Indexers.'),
    dataIndex: 'stakedTokens',
    key: 'stakedTokens',
    render: renderFormattedValue,
  },
  {
    title: createTitleWithTooltipDescription(titles.proportion),
...
export const transformToRows = (favourites: Map<string, number>) => (subgraphs: Array<Subgraph>) => {

...
type AccountSearchResponseLite = Record<'accounts' | 'indexers' | 'delegators' | 'curators', Array<Account>>;

const getAccountsLite = async (searchTerm: string, first = 5) => {
  const { accounts, indexers, delegators, curators } = await request<AccountSearchResponseLite>(gql`
    query {
  ...
  return uniqBy(prop('id'), [...accounts, ...indexers, ...delegators, ...curators]);
};

export const useAccountsSearch = (searchTerm: string, isLite: boolean) => {
  const debouncedSearchTerm = useDebounce(searchTerm, 500);

  return useQuery(
    ['accounts-search', debouncedSearchTerm],
    async () => {
      const accounts = await (isLite

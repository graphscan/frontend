...
export const useAllocationsRewards = () => {
  const [allocationsRewards, setAllocationsRewards] = useState<AllocationsRewards>({});

  const onRewardsReceived = useCallback((id: string, potentialRewards: PotentialRewards) => {
  ...
  const renderRewardsButton = useCallback(
    (allocationId: string, potentialIndexingRewardCut: number) =>

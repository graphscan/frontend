...
export const useFavouriteColumn = <T extends { id: string }>({
  ...
  const initialFavourites = useMemo(() => new Map(Object.entries(defaultFavourites ?? {})), [
    defaultFavourites,
  ]);
    ...
    (id: string) => (e: ChangeEvent<HTMLInputElement>) => {
      setFavourites((prevState) => {
        ...
        if (typeof localStorage !== 'undefined') {
          localStorage.setItem(favouriteStorageKey, JSON.stringify(Array.from(prevState)));
        }
  ...
  const handleSetIsFavouritesFixed = useCallback((e: ChangeEvent<HTMLInputElement>) => {
  ...
  const columns = useMemo(
    () => [
      {
        title: Checkbox({ checked: isFavouritesFixed, onChange: handleSetIsFavouritesFixed }),
        dataIndex: 'favourite',
        key: 'favourite',
        render: (isFavourite: boolean, row) =>
          Checkbox({ checked: isFavourite, onChange: handleSetFavourite(row.id) }),
      },
      ...initialColumns,
    ],
    [handleSetFavourite, handleSetIsFavouritesFixed, initialColumns, isFavouritesFixed],
  );
